...
void PacketFilter_PSClassic::filter_packet(Packet* packet) {
	if (packet->wLength >= 2) {
		printf("%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d\n", 
			...
			(packet[0] & 0x0100000000000000) == 1, 
			(packet[0] & 0x0010000000000000) == 1, 
			(packet[0] & 0x0001000000000000) == 1, 
			(packet[0] & 0x0000100000000000) == 1, 
			(packet[0] & 0x0000010000000000) == 1, 
			(packet[0] & 0x0000001000000000) == 1, 
			(packet[0] & 0x0000000100000000) == 1, 
			0,
			0, 
			(packet[0] & 0x0000000000100000) == 1, 
			(packet[0] & 0x0000000000010000) == 0, 
			(packet[0] & 0x0000000000001000) == 1, 
			(packet[0] & 0x0000000000000100) == 0, 
			(packet[0] & 0x0000000000000010) == 1, 
